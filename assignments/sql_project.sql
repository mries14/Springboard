...
JOIN ( SELECT * 
        FROM  `Bookings` 
        WHERE starttime LIKE  "2012-09-14%"
     ) sub ON sub.bookid = Bookings.bookid
HAVING cost > 30
ORDER BY cost DESC 
...
SELECT Facilities.name,
       sub.cost - initialoutlay -3 * monthlymaintenance AS total_revenue
FROM Facilities
JOIN (SELECT Facilities.name AS facility_name,
     ...
     ) sub ON Facilities.name = sub.facility_name
